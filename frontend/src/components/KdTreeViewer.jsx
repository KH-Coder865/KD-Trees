...
//   return (
//     <>
//       <div className="bg-white shadow-2xl flex flex-col justify-center rounded-xl p-6 md:p-8 h-auto w-[88vw]">
...
//             <div className="bg-gray-50 p-5 rounded-lg border border-gray-200 shadow-xl">
...
// --- Inline SVG Icons ---
const Icon = ({ children, className = "w-6 h-6", strokeWidth = 2, viewBox = "0 0 24 24", ...props }) => (
  <svg xmlns="http://www.w3.org/2000/svg" className={className} width="24" height="24" viewBox={viewBox}
    fill="none" stroke="currentColor" strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round" {...props}>
    {children}
  </svg>
);
const Network = (props) => (<Icon {...props}><path d="M1 21h4V9H1v12zm8 0h4V3H9v18zM17 21h4v-6h-4v6z" /></Icon>);
const Map = (props) => (<Icon {...props}><path d="M1 6v15m0-10l5 5l5-5l5 5l5-5v10l-5 5l-5-5l-5 5l-5-5z" /></Icon>);
const Zap = (props) => (<Icon {...props}><path d="M13 2L3 14h9l-1 8l10-12h-9l1-8z" /></Icon>);
const RefreshCw = (props) => (<Icon {...props}><path d="M23 4v6h-6m6 0l-3-3m3 3l-3 3M1 20v-6h6m-6 0l3 3m-3-3l3-3m3-10a10 10 0 0 1 1.7-6.2A10 10 0 0 1 12 2a10 10 0 0 1 10 10a10 10 0 0 1-10 10a10 10 0 0 1-10-10a10 10 0 0 1 1.7-6.2" /></Icon>);
const Search = (props) => (<Icon {...props}><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></Icon>);
const Layers = (props) => (<Icon {...props}><path d="M12 2l10 5l-10 5l-10-5l10-5zM2 17l10 5l10-5M2 12l10 5l10-5" /></Icon>);
...
const useDebouncedLoading = (delay = 300) => {
  ...
  const start = useCallback(() => {
    setIsPending(true);
    clearTimeout(timerRef.current);
    timerRef.current = setTimeout(() => setShowOverlay(true), delay);
  }, [delay]);
...
// --- Binary Tree Viewer (added) ---
const BinaryTreeNode = ({ node, x, y, level, parentX }) => {
  if (!node) return null;
  const childY = y + 80;
  ...
  return (
    <>
      ...
      {/* node circle */}
      <circle cx={x} cy={y} r="15" fill="#3b82f6" stroke="white" strokeWidth="2" />
      <text x={x} y={y + 5} textAnchor="middle" fontSize="10" fill="white">
        ({node.point[0]}, {node.point[1]})
      </text>
      {/* children */}
      {node.left && (
        <BinaryTreeNode node={node.left} x={x - offset * 3} y={childY} level={level + 1} parentX={x} />
      )}
      {node.right && (
        <BinaryTreeNode node={node.right} x={x + offset * 3} y={childY} level={level + 1} parentX={x} />
      )}
    </>
  );
};

const BinaryTreeViewer = ({ treeStructure }) => {
  if (!treeStructure) {
    return <p className="text-gray-500 text-sm">Binary tree structure not available yet.</p>;
  }
  return (
    <div className="p-4 bg-white border rounded-lg shadow-sm overflow-x-auto">
      <svg width="600" height="400" viewBox="0 0 600 400">
        <BinaryTreeNode node={treeStructure} x={300} y={40} level={1} parentX={null} />
      </svg>
    </div>
  );
};
...
const KdTreeViewer = () => {
  ...
  const fetchData = useCallback(async () => {
    start(); setApiError(null);
    try {
      const [pointsData, structureData] = await Promise.all([
        useApi('/traverse', 'GET'),
        useApi('/structure', 'GET')
      ]);
      if (pointsData.error) setApiError(`Failed to load points: ${pointsData.error}`);
      else if (structureData.error) setApiError(`Failed to load tree structure: ${structureData.error}`);
      else {
        setPoints(pointsData.points.map(([x, y], index) => ({ id: index, x, y })));
        setTreeStructure(structureData.tree);
      }
    } catch (error) { setApiError(`Error: ${error.message}`); } finally { stop(); }
  }, [start, stop]);
  ...
  const drawPartitions = useCallback((ctx, node, minX, maxX, minY, maxY) => {
    ...
    if (dim === 0) {
      ...
      const pYmin = coordToPixel(minY, true);
      const pYmax = coordToPixel(maxY, true);
      ctx.strokeStyle = `rgba(239,68,68,${0.4 + (depth * 0.05)})`;
      ...
      const pXmin = coordToPixel(minX);
      const pXmax = coordToPixel(maxX);
      ctx.strokeStyle = `rgba(59,130,246,${0.4 + (depth * 0.05)})`;
  ...
  const draw = useCallback(() => {
    ...
    for (let i = 0; i <= 10; i++) {
      ctx.beginPath(); ctx.moveTo(0, i * 70); ctx.lineTo(CANVAS_SIZE, i * 70); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(i * 70, 0); ctx.lineTo(i * 70, CANVAS_SIZE); ctx.stroke();
    }
    if (treeStructure) drawPartitions(ctx, treeStructure, BOUNDARY_MIN, BOUNDARY_MAX, BOUNDARY_MIN, BOUNDARY_MAX);
    points.forEach(p => {
      const px = coordToPixel(p.x);
      const py = coordToPixel(p.y, true);
      ctx.fillStyle = '#10b981';
      ctx.beginPath(); ctx.arc(px, py, 5, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = '#4b5563'; ctx.font = '10px Inter'; ctx.fillText(`(${p.x}, ${p.y})`, px + 7, py + 4);
    });
  }, [points, treeStructure, drawPartitions]);
  ...
  return (
    <div className="bg-white shadow-2xl rounded-xl p-6 md:p-8 w-[88vw]">
      <h1 className="text-3xl font-extrabold text-gray-800 mb-2 flex items-center">
        <Network className="mr-3 w-7 h-7 text-emerald-600" />
        KD-Tree Interactive Viewer
      </h1>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        {/* Left: Canvas */}
        <div className="w-full">
          <canvas ref={canvasRef} width={CANVAS_SIZE} height={CANVAS_SIZE} className="border-4 border-gray-200 rounded-lg shadow-lg bg-gray-50" />
        </div>

        {/* Right: Docs + Binary Tree Visualization */}
        <div className="w-full md:w-1/2 space-y-6">
          <div className="bg-gray-50 p-5 rounded-lg border border-gray-200 shadow-xl">
            <h2 className="text-xl font-bold text-gray-800 mb-4">KD-Tree Concepts</h2>
            <div className="flex mb-4 border-b border-gray-300">
              <button
                className={`flex-1 py-2 text-sm font-medium ${activeTab === 'legend' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500'}`}
                onClick={() => setActiveTab('legend')}
              ><Layers className="w-4 h-4 inline mr-1" /> Partitions</button>
              <button
                className={`flex-1 py-2 text-sm font-medium ${activeTab === 'delete' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500'}`}
                onClick={() => setActiveTab('delete')}
              ><Trash2 className="w-4 h-4 inline mr-1" /> Deletion</button>
            </div>
            ...
            <div className="text-sm text-gray-700 space-y-3">
              {activeTab === 'legend' ? (
                <>
                  <h3 className="font-bold text-base text-gray-800">Space Partitioning</h3>
                  <p>The visualization now shows the **splitting hyperplanes** used by the tree:</p>
                  <ul className="list-disc list-inside space-y-1 ml-2">
                    <li><span className="text-red-600 font-semibold">Red Lines</span>: X-dimension splits.</li>
                    <li><span className="text-blue-600 font-semibold">Blue Lines</span>: Y-dimension splits.</li>
                  </ul>
                </>
              ) : (
                <>
                  <h3 className="font-bold text-base text-gray-800">Point Deletion</h3>
                  <p>Deletion replaces a node with the minimum in its split dimension from its right subtree.</p>
                </>
              )}
            </div>
          </div>

          {/* --- Added Binary Tree Visualization Section --- */}
          <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-lg">
            <h2 className="text-xl font-bold text-gray-800 mb-3">Binary Tree Representation</h2>
            <p className="text-gray-600 text-sm mb-3">
              This diagram shows the binary (hierarchical) representation of the same KD-tree.
            </p>
            <BinaryTreeViewer treeStructure={treeStructure} />
          </div>
        </div>
      </div>
    </div>
